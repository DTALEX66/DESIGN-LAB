...
2. inspect `.extract-packages/design-system/normalized.json`
3. import `packages/design-system/tokens.css` into the app when the user is ready
...
Use `npx extract-design-system init` only when `.extract-packages/design-system/normalized.json` already exists and the user wants to regenerate token files.
