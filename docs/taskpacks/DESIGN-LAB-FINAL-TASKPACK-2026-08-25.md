...
- 基线核验时间：`2026-08-26 UTC`
- 状态：`READY_FOR_OPEN_DESIGN_HOST_ADOPTION`
- 前序包：`DLR-FINAL-20260825-R1`（被本包取代，保留为历史证据）
...
Open Design 是 DESIGN-LAB 当前正式主宿主、主界面和通用运行入口，但不是 DESIGN 的专业真值或产品所有权。DESIGN 以官方插件/CLI/MCP 扩展形成下游发行层；MiniGame 是游戏视觉 Domain/回归 fixture，不是项目运行时产品线。
...
- Open Design 上游发展极快，当前官方仓库已是完整多宿主系统；升格为主产品底座，但只能锁发布版、走官方扩展面和维护窄 patch queue，不可散乱 vendoring 或追随 `latest`。
...
|---|---|---|---|---|
| [Open Design](https://github.com/nexu-io/open-design) | Apache-2.0；完整 Windows/CLI/MCP/插件/预览与导出 | `ADOPT_PRIMARY_HOST` | 主界面、Agent/CLI/MCP、插件、沙箱预览、HTML/PDF/PPTX/MP4 交付 | DESIGN 真值仅经插件/API；不复制其全部 skills，不读取私有 DB |
...
| [OpenPencil](https://github.com/open-pencil/open-pencil) | MIT；active development、CLI/SDK/MCP 完整 | `ADOPT_EDITABLE_CANVAS` | 本地 `.fig/.pen`、node tree、components/variants、auto-layout、结构化读写 | 锁版本和黄金格式；不在 DESIGN 自研第二画布 |
...
### DLR-070 — Open Design 主宿主采用与 DESIGN 核心插件

- 优先级/状态：`P0 / BLOCKED_RUNTIME`
- 依赖：DLR-010、DLR-030、DLR-040。
- 流程：锁发布版/tag/commit→原样 Windows 黄金流程→创建 `design-lab-core` 官方格式插件→映射 Brief/Direction/DesignIR/HumanGate/Jury/Rights/Preflight→真实 brief→artifact/editable source→provenance/readback→失败/恢复→卸载/rollback。
...
### DLR-080 — OpenPencil 主可编辑画布与 Penpot 可选协作 lane

- 优先级/状态：`P1 / CONDITIONAL`
- 依赖：DLR-060、DLR-070。
- OpenPencil 作为第一版正式可编辑 UI/vector lane，验证结构读写、components/tokens、constraints/auto-layout、`.fig/.pen/SVG/PDF/HTML-CSS`、离线、Windows、MCP/SDK 权限、备份、性能、许可与退出。
- 安全：Penpot MCP 的代码执行置于专用文件/最小权限/timeout；OpenPencil 的 active-development rough edges 进入风险矩阵。
- Penpot 仅在真实多人协作需求出现时启用为 `OPTIONAL_COLLAB_ADAPTER`；不与 OpenPencil 双主、不 vendoring 两套编辑器。
...
### DLR-130 — ComfyUI 外置生成 runtime 重验与单选

- 优先级/状态：`P2 / CONDITIONAL`
- 依赖：DLR-100 证明局部编辑/生成存在真实缺口。
- 第一候选：复用历史已建设 Adapter 的 ComfyUI，但按当前版本和 8GB VRAM 重新资格化；比较蒙版/局部编辑/ControlNet、batch、参数可重复、Windows、license、offline、API、provenance 与 editable handoff。
- 后备候选：InvokeAI 或 Krita AI Diffusion，只在 ComfyUI 48 小时资格化失败时进入；不双生产。
- 外置：GPL 程序、模型权重、安装器、缓存和用户素材不进入核心仓库；H3 继续冻结。
...
- 禁止把 DESIGN-LAB 的专业真值改成 Open Design 私有数据库，或再造第二画布、通用 Agent/聊天/知识 OS；允许并要求以 Open Design 官方插件/锁版本下游层作为产品主入口。
...
研究时间点更新为 2026-08-26；执行时必须锁不可变 commit/tag、逐项复核权重与数据许可，且不得以 `latest` 作为供应链证据。

## 13. R2 OSS 快速成品化增补（取代与其冲突的 R1 条款）

### 13.1 唯一产品路线

```text
Open Design primary host / product UI
                  |
       design-lab-core plugin
                  |
Brief / DesignIR / HumanGate / Jury / Rights / Preflight
                  |
OpenPencil + Adobe UXP + native-tool adapters
                  |
 Editable artifact / BOM / Provenance / Receipt
```

- Open Design 承担通用桌面入口、Agent/CLI/MCP、插件、预览与常规导出；DESIGN 不再自研完整前端。
- OpenPencil 承担可编辑 UI/vector 画布；Adobe、Blender、Inkscape、Krita 等继续通过官方/原生外置接口交付。
- DESIGN 只自研 `design-lab-core` 插件、Design IR、Domain Pack、Human Gate、专业 Jury、rights、preflight 和 Adapter/Receipt。
- 上游 skills、design systems 和 plugins 只有经 DESIGN capability manifest、许可、hash、消费者和测试审核后才算本项目能力。

### 13.2 72 小时执行单

1. 锁 Open Design、OpenPencil、ComfyUI 和至少一个官方宿主插件 starter 的版本、commit/hash、SPDX、数据目录和权限。
2. 在 Windows 专用测试项目原样跑通 Open Design brief/preview/export 和 OpenPencil node edit/close/reopen/readback。
3. 创建最薄 `design-lab-core` 插件：只实现 Brief、Direction、Rights、Quality、Production 和 Release gate 的读取/提交，不迁移全部历史能力。
4. 选择一个 UI 或品牌真实 brief，产出 OpenPencil 可编辑源、预览、BOM、rights 和 receipt；至少一次人审拒绝后修复。
5. 用 Adobe UXP 官方 sample 或 Inkscape/Blender 官方接口完成一次原生宿主修改；ComfyUI 只在需要生成素材时外置调用。
6. 验证所有第三方 `AGENTS/CLAUDE/install/affiliate` 不进入 Open Design 插件发现和 prompt。

### 13.3 14 天发布门

发布 `DESIGN-LAB 0.1-usable` 必须证明：

- Open Design 能从真实 brief 进入 DESIGN 插件流程；
- Direction/Rights/Quality/Production/Release gate 无法由模式或直接 API 绕过；
- OpenPencil 或一个专业宿主产出原生可编辑文件，关闭重开结构一致；
- 至少一次真实人工 REJECT、修改和回归通过；
- 交付含 editable source、preview、BOM、rights、quality/preflight、provenance 和 rollback；
- 生成模型缺失、8GB VRAM 不足、宿主未安装、MCP 拒绝和取消均诚实失败；
- Windows 安装、升级、备份、卸载、SBOM/NOTICE 完整。

第一条黄金闭环通过前，冻结第二画布、第二主宿主、第二生成 runtime，以及品牌/UI/包装/视频/3D/出版多线并行。
