...
const required = [
  ...
  join(pluginRoot, ".github/ISSUE_TEMPLATE/skill-request.yml"),
  ...
  join(pluginRoot, "skills/catalog.json"),
...
if (plugin.repository !== "https://github.com/OpaceDigitalAgency/skills") {
...
if (claudePlugin.repository !== "https://github.com/OpaceDigitalAgency/skills") {
...
const catalog = JSON.parse(
  await readFile(join(pluginRoot, "skills/catalog.json"), "utf8"),
);
const catalogSkill = catalog.skills?.find(
  (entry) => entry.name === "ai-ui-ux-motion-engine",
);
if (!catalogSkill) {
  failures.push("Skill catalogue is missing ai-ui-ux-motion-engine.");
} else {
  if (catalogSkill.status !== "available") {
    failures.push("AI UI/UX Motion Engine is not marked available in the catalogue.");
  }
  if (
    catalogSkill.repository !==
    "https://github.com/OpaceDigitalAgency/ai-ui-ux-motion-engine"
  ) {
    failures.push("Skill catalogue standalone repository URL is incorrect.");
  }
  if (catalogSkill.version !== "1.6.4") {
    failures.push("Skill catalogue version is not 1.6.4.");
  }
}
...
await validateLocalMarkdownLinks(join(pluginRoot, "skills/README.md"));
