# PROJECT_STATUS（生成状态，DL-MIG-005）

- 生成时间：2026-08-16｜生成器：`scripts/generate_project_status.py (DL-MIG-005)`
- Git：分支 `fix/design-lab-governance-closure-r4`，HEAD `078b451e2581`，origin/main `b9f69acd13f5`，worktree dirty
- tracked 文件：2927；仓库 pack：198.36 MiB（预算 256 MiB，预警 220 MiB）

## 来源治理
- 活动登记（v3）：0；隔离登记：162（162 条遗留已隔离，DL-KNW-003）

## 能力与证据
- 能力索引：2239 项；证据卡：12（authoritative accepts=0）
- 各能力证据等级分布：E1=8

## 资产
- tracked 二进制：51（9.87 MiB，全量清单见 DL-AST-002-BINARY-INVENTORY.json）

## 测试
- 定义测试方法数（AST 静态计数）：95
- 实际执行结果以 `python scripts/run_python_tests.py` 为准（本文件不冒充执行证据）

> 活动文档只引用本生成状态，不复制数字（DL-MIG-005）。
