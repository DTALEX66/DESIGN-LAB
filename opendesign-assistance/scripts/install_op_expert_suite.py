...
SUITE_SRC = REPO_ROOT / "opendesign-assistance" / "op-expert-suite" / "skills"
...
DESIGN_SYSTEM_MANAGED_PREFIX = "Managed by OPEN-DESIGN-Assistance; slug="
...
def begin_stable_resource_source_update(app_url: str) -> StableSourceUpdate:
    ...
    target = namespace_data_root(app_url) / "local-plugin-sources" / "open-design-assistance"
    target.parent.mkdir(parents=True, exist_ok=True)
    temp = Path(tempfile.mkdtemp(prefix=".open-design-assistance-", dir=target.parent))
...
def install_expert_resource(
    ...
    source = f"./../data/local-plugin-sources/open-design-assistance/{kind}/{resource_id}"
    opener = urllib.request.build_opener(urllib.request.ProxyHandler({}))
...
def verify_expert_resources(app_url: str, headers: dict[str, str]) -> None:
    ...
    for kind, resource_id in EXPERT_RESOURCE_SOURCES:
        ...
        expected = (
            ...
            f"open-design-assistance/{kind}/{resource_id}"
        )
    ...
    for spec in DESIGN_SYSTEM_SOURCES:
        ...
        if current:
            ...
            managed = (
                ...
                or "maintained by OPEN-DESIGN-Assistance" in source_notes
            )
            if not managed:
